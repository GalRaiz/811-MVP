...
const AssistanceFormFinalStep: React.FC = () => {
  ...
  const handleFieldChange = (fieldName: 'requestName' | 'requestDescription', value: string) => {
  ...
  return (
    <div className="assistance-form">
      ...
      <div className="assistance-form__fields">
        {fields.map(field => (
          <FormField
            ...
            type={field.type}
            value={field.value}
            onChange={(value) => handleFieldChange(field.id as 'requestName' | 'requestDescription', value)}
          />
        ))}
      </div>
      ...
      <div className="assistance-form__instructions">
        <p>
        ככל שנדע יותר על הבקשה במדויק, כך יצטמצם זמן הטיפול בה. 
        </p>
      </div>
    </div>
  );
