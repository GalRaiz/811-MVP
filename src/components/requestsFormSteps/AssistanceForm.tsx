...
const AssistanceForm: React.FC = () => {
  ...
  const handleFieldChange = (fieldName: 'requesterName' | 'requesterPhone' | 'requestName', value: string) => {
    ...
    <div className="assistance-form">
      ...
      <div className="assistance-form__fields">
        {fields.map(field => (
          <FormField
            ...
            type={field.type}
            value={field.value}
            onChange={(value) => handleFieldChange(field.id as 'requesterName' | 'requesterPhone' | 'requestName', value)}
          />
        ))}
      </div>
      ...
      <div className="assistance-form__instructions">
        <p>
          ...
          אנחנו רק רוצים לדעת למי בדיוק להפנות את הסיוע. מספר הטלפון צריך להיות של מישהו שנמצא עם המקבלים 
          ויכול לענות לשליח או למתנדב בשטח.
        </p>
      </div>
    </div>
  );
